...
const TakeAttendance = () => {
  ...
  const filteredChildren = children.filter((child) => {
    ...
    const uniqueId = (child.uniqueId || '').toLowerCase();
    return fullName.includes(query) || uniqueId.includes(query);
  });
      ...
      {loading ? (
        ...
        <form onSubmit={handleSubmit}>
          ...
          {filteredChildren.length === 0 ? (
            <div className="card text-center py-8">
              <p className="text-gray-500">No children match your search "{searchQuery}"</p>
              <button
                type="button"
                onClick={() => setSearchQuery('')}
